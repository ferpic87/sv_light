(function ($)
{
    ...
    TagsMaterialize.prototype = {
        ...
        {
            var self = this;
			      self.itemsArray = getIngredients();
            ...
            // add a tag element
            var $tag = $('<span class="' + htmlEncode(tagClass) + (itemTitle !== null ? ('" title="' + itemTitle) : '') + '">' + "<div class=\"tt-suggested-tag\" style='background-image: url("+getIngredientImage('img/ingredienti/'+removeSpecialChars(itemText)+'.png')+");' /><span>"+htmlEncode(itemText) + '</span><i class="material-icons" data-role="remove">close</i></span>');
            ...
            */
			      ...
			      $tag.data('item', item);
            self.findInputWrapper().parent().after($tag);
            $tag.after(' ');
            ...
            self.$element.trigger($.Event('itemAdded', {item : itemText, options : options}));
        },
        ...
        remove : function (item, dontPushVal, options)
        {
            ...
            if (item)
            {
                ...
                $('.chip', self.$container.parent()).filter(function () { return $(this).data('item') === item; }).remove();
        ...
        build : function (options)
        {
            ...
            // Remove icon clicked
            self.$container.parent().on('click', '[data-role=remove]', $.proxy(function (event)
            {
                if (self.$element.attr('disabled'))
                {
                    return;
